# skill-router — routing tables and protocols (v3 reference)

Moved out of SKILL.md in v4 so the skill body stays under 450 words. The
route card carries what the model needs per turn; this file is the full
contract for reading and for editing the tables.
...
## IT LEARNS — AND THE LEARNING LIVES OUTSIDE THE REPO

Everything above is generic. What makes it yours accumulates in one file that
is never committed and never hand-edited:

```
~/.claude/skill_router_learned.json      regenerated by scripts/learn.py at every session start
```

The engine is versioned and shared; the overlay is personal and regenerated.
Nothing learned is ever written into SKILL.md or SKILL.personal.md — those
stay hand-curated. Earlier tooling appended learned chains into the personal
file, which made every relearn a merge; that path is retired.

Four signals, each with a support-and-precision floor (a pattern seen twice is
a coincidence), each surfaced where it is useful rather than in a report:

| Learned | From | Surfaces as |
|---|---|---|
| **Triggers** — keyword → skill | prompts where you then invoked a skill | `[skill-router] Learned from your history: prompts with testflight, submit usually use scrollbook-deploy.` — only when the table and matcher are both silent |
| **Handovers** — what follows what | consecutive Skill calls in a session | after a Skill runs: `[skill-router] After writing-plans you usually run test-driven-development (71%, n=7).` |
| **Chains** — recurring 3–4 step flows | n-grams over sessions | at announcement: `Your usual flow from here: tdd → verification` |
| **Follow rates** — announced vs invoked | announcement / invoke joins by prompt id | the router refuses to resurrect a skill you routinely ignore |

Plus two forms of **discovery**, shown as at most three lines at session start:

```
[skill-router] New since last session: scrollbook-qa, seo-hreflang (+3) — the router can announce these now.
[skill-router] Fits your recent work, not installed: stripe-webhooks (antigravity) ← stripe, checkout, webhooks
[skill-router] Your usual flow: superpowers:writing-plans → test-driven-development → verification (×7)
```

- **Installed** — the catalog is diffed against the last run, so a skill you
  add is announced at your next session and then never again.
- **Online** — the four known catalogs (2,400 skills) are refetched weekly in
  the background and ranked against *your* recent prompt keywords, not offered
  as a list. Nothing is suggested until 20 prompts of signal exist, because a
  profile built from skill names alone recommends frameworks to everyone.

All of it is advisory. Learned associations never carry the IRON RULE: they
are statistics about your habits, and enforcing a habit turns a nudge into a
cage. What gets logged is prompt **keywords**, never prompt text; set
`SKILL_ROUTER_NO_LEARN=1` to log nothing.

```bash
python3 ~/.claude/skills/skill-router/scripts/learn.py         # relearn now, print what it found
python3 ~/.claude/skills/skill-router/scripts/learn.py --show  # dump the overlay
```
...
| `scripts/learn.py` | relearn the personal overlay now (runs itself at every session start) |
| `scripts/learn.py --show` | inspect what has been learned |
